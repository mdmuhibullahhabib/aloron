  ...
  <div className="p-8">
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {subjects.map(subject => (
        <div
          key={subject.id}
          className="flex flex-col items-center justify-center p-6 bg-gray-100 rounded-xl cursor-pointer hover:bg-gray-200 transition-colors"
        ...
        >
          ...
          <span className="text-lg md:text-xl font-medium text-gray-800">{subject.name}</span>
        </div>
      ))}
    </div>
    ...
    <div className="mt-8">
      ...
      <div className="flex flex-wrap gap-2">
        {presetExams.map(exam => (
          <button key={exam} className="px-4 py-2 bg-green-200 text-gray-800 rounded-lg hover:bg-green-300 transition-colors">{exam}</button>
        ))}
      </div>
    </div>
  </div>
