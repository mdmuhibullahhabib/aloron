...
const JournalDetails = () => {
  ...
  const [journals, setRelated] = useState([]);


  useEffect(() => {
    const fetchJournal = async () => {
      try {
        const res = await axiosSecure.get(`/journals/${id}`);
        setJournal(res.data);

        // Related journals fetch (same category/journal type)
        const relatedRes = await axiosSecure.get(
          `/journals?journal=${res.data.journal}&limit=3`
        );
        setRelated(relatedRes.data.filter((j) => j._id !== res.data._id));
      } catch (error) {
