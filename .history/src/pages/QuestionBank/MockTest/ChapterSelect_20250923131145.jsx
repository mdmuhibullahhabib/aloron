...
const ChapterSelect = ({ selectedSubject, onConfirmChapters, onGoBack }) => {
  ...
  return (
    <div className="p-8 bg-white rounded-xl shadow-md">
      <button onClick={onGoBack} className="mb-6 text-gray-600 hover:text-green-500 flex items-center gap-2">
        ← Back
      </button>
      <h2 className="text-2xl font-bold mb-4 text-gray-800">{selectedSubject.name}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {chaptersForSubject.map(chapter => (
          <label key={chapter} className="flex items-center gap-3 p-4 bg-gray-100 rounded-xl cursor-pointer hover:bg-green-100">
            <input
              type="checkbox"
