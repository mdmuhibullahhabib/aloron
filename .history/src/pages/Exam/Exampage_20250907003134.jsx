...
const Exampage = () => {
  ...
  // Custom Hook থেকে ডাটা
  const [questionSet, refetch, isLoading] = useExamQuestion(selected);

  // Selected পরিবর্তন হলে নতুন ডেটা আনবে
  useEffect(() => {
  ...
  const handleAnswer = (ans) => {
    if (ans === questionSet.questions[currentQ - 1].answer) {
      setScore(score + 1);
    }
    if (currentQ < questionSet.questions.length) {
    ...
    return (
      <div className="max-w-2xl mx-auto p-6 text-center">
        ...
        <div className="bg-white shadow-lg rounded-lg p-6 border">
          <p className="text-lg">Total Questions: {questionSet?.questions?.length}</p>
          <p className="text-lg">Correct: {score}</p>
          <p className="text-lg">Wrong: {questionSet?.questions?.length - score}</p>
          <p className="text-lg font-bold">
            Score: {((score / (questionSet?.questions?.length || 1)) * 100).toFixed(2)}%
          </p>
        </div>
        <button
  ...
  if (started) {
    const q = questionSet?.questions?.[currentQ - 1];
    return (
      <div className="max-w-2xl mx-auto p-6">
        <h2 className="text-xl font-semibold mb-4">
          প্রশ্ন {currentQ} / {questionSet?.questions?.length}
        </h2>
        <div className="bg-white shadow-lg rounded-lg p-6 border">
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <button
        ...
        disabled={!selected.chapter || !questionSet?.questions?.length}
